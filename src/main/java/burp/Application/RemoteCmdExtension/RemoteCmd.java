...
public class RemoteCmd {
    ...
    private Integer getMaxExecutionTime() {
        ...
        Integer payloadSize = this.yamlReader.getStringList("application.remoteCmdExtension.config.payloads").size();
        Integer parameterSize = this.analyzedRequest.getEligibleParameters().size();
        maxExecutionTime += (payloadSize * 6) + (parameterSize * 6);
